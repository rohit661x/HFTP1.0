...
# MoldUDP64 Format: [8B Session ID][4B Length][1B msg_type][2B stock_locate][4B price][4B shares]
...
def make_raw_packet(session_id, msg_type, stock_locate, price, shares,
    ...
    Construct a raw MoldUDP64 packet:
      ...
      - 11-byte payload (little-endian):
          [1B msg_type][2B stock_locate][4B price][4B shares]
    Options:
      malformed: sets payload length > allowed buffer size (for error testing)
      corrupt: flips bits in the first byte of the payload
    """
