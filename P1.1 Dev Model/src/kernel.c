...
static inline uint8_t inb(uint16_t port) {  //reads 1 byte (8-bit) from 16-bit port
    uint8_t ret;
    __asm__ volatile ("inb %1, %0" : "=q"(ret) : "d"(port));
    //in line assembly command in C:     %0 = first operand and %1 = secound operand
    //                                   "q" = use any 8-bit register
    //                                   "d" = use dx register 
    return ret;
}

//sends 1 byte to 16-bit port
static inline void outb(uint16_t port, uint8_t val) {
    __asm__ volatile ("outb %0, %1" : "=q"(val) : "a"(val), "d"(port));
}
...
void pci_enumerate() {                                                             // scan every possible PCI bus (0-255), device slot (0-31), and function number (0-7)
    ...
    if (nic_mmio_base == 0) {
        serial_write("No NIC found; stubbing NIC MMIO base to 0x00500000\r\n"); // for debugging will remove in performance model
...
typedef struct {
    uint64_t addr;              // pointer to packet buffer, 64-bit physical address where the NIC should DMA the incoming packet
    uint32_t length;            // size of that buffer
    uint8_t  status;            // Descriptor Done (DD) bit is bit 0
    uint8_t  pad[7];            // pad out to 16 bytes, improves cache efficiency 
} rx_desc_t;
...
// Input interface from Verilog Parser to C driver
typedef struct {
    char     msg_type;                         // identfies message subtype ex. 'A' for Add-Order
    uint16_t stock_locate;                     // the raw code (16-bit) from RTL for the security, match thins against AAPL_LOCATE
    uint32_t price;                            // price in ticks
    uint32_t shares;                           // share quantity of security
    //ADD FIELDS AS NEEDED TO MATCH OUTCH             
} hw_msg_small;
...
void init_rx_ring(void) {
    ...
    nic_write32(NIC_REG_RXDESC_HI,   (uint32_t)(RX_DESC_BASE >> 32));               // Upper 32 bits if the ring base address
...
void init_tx_ring(void) {
    ...
    nic_write32(NIC_REG_TXDESC_HI,   (uint32_t)(TX_DESC_BASE >> 32));               // Shifts right 32 bits, to get the full 64-bit message
...
void poll_rx_loop(void) {
    ...
    while (true) {                                                          // entering infinite spin loop
        ...
        uint32_t next_rx_tail = (rx_tail + 1) % RX_RING_SIZE;               // compute the next tail index if NIC wrote one more packet
        ...
        while (rx_head != rx_tail) {                                                // loop from head up to but not including tail, processing each descriptor only once
            if (__builtin_expect(desc[rx_head].status & RX_DESC_STAT_DD, 0)) {      // check DD bit of the descriptor
                ...
                if (pkt_len > PKT_BUF_SIZE) {                                       // if packet is larger than buffer size then drop it 
                    ...
                    rx_head = (rx_head + 1) % RX_RING_SIZE;                         // bump rx_head and continue to skip handeling
                    continue;
                }

                desc[rx_head].status = 0;                                   // clear DD bit so NIC can reuse this entry

                hw_msg_small *msg = (hw_msg_small*)desc[rx_head].addr;      // call message handler for valid packet
                handle_msg(msg);                                            // calls handle_msg, thus we begin processing
            }
            
            rx_head = (rx_head + 1) % RX_RING_SIZE;                         // Move to next descriptor, increment rx_head so each descriptor is visited once per pass
        }
